...
# A script to automate the publishing of the hangarbay package to PyPI.
# Exit immediately if a command exits with a non-zero status.
set -e

echo "Starting the hangarbay publishing process..."
echo "========================================"
...
rm -rf build dist hangarbay.egg-info
...
select choice in "Publish to TestPyPI" "Publish to PyPI (Official)" "Cancel"; do
    case $choice in
        "Publish to TestPyPI" )
            ...
            echo "You can view the package at: https://test.pypi.org/project/hangarbay/$VERSION/"
            echo "You can install it using: python3 -m pip install --index-url https://test.pypi.org/simple/ --extra-index-url https://pypi.org/simple hangarbay==$VERSION"
            break
            ;;
        "Publish to PyPI (Official)" )
            ...
            if [[ $REPLY =~ ^[Yy]$ ]]; then
                ...
                echo "Your package is now live at: https://pypi.org/project/hangarbay/$VERSION/"
                ...
                if [[ $REPLY =~ ^[Yy]$ ]]; then
                    if [ "$HAS_GH" = "false" ]; then
                        ...
                        echo "Please create the release manually: https://github.com/stiles/hangarbay/releases/new"
                        exit 0
                    fi
